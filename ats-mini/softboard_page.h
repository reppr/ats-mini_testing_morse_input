...
void pins_info_analog() {
  ...
  void pins_info_touch() {
    ...
    for (i=0; i<40; i++)	// TODO: fix range of touch pins
      pin_info_touch(i);
...
#if defined ESP32	// touch interface?
  #undef has_ARDUINO_TONE	// ##### TODO: clashes with 'T' for Arduino style tone();
  case 'T':	// ################ TODO: clashes with 'T' for Arduino style tone();
    newValue = MENU.numeric_input(PIN_touch);
    if (newValue>=0 && newValue<ILLEGAL8) {	// TODO: fix range of touch pins
      PIN_touch = newValue;
    } else {
      MENU.OutOfRange();			// TODO: fix range of touch pins
    }

  #if defined PULSES_SYSTEM
    if(PIN_touch == ILLEGAL8)
      PIN_touch = HARDWARE.morse_touch_input_pin;
  #endif
